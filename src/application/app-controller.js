...
function publicError(error) {
  return { code: error?.code ?? 'UNKNOWN_ERROR', message: error?.message ?? 'Неизвестная ошибка.' };
}

export function createAppController({ repository, exporter, idFactory, clock, projectFactory = createDefaultProject }) {
  const listeners = new Set();
  let state = {
    project: projectFactory({ id: idFactory(), now: clock() }),
    dirty: true,
    status: 'ready',
    error: null,
    pendingIntent: null,
  ...
  function dispatch(result) {
    ...
    if (!result.changed) {
      if (result.confirmationRequired) update({ pendingIntent: { type: 'resize', result }, error: null });
  ...
  function save() {
    ...
    try {
      ...
      try { state.projects = repository.list(); } catch { /* successful save remains valid */ }
      notify();
  ...
  function performIntent(intent) {
    if (intent.type === 'new') {
      update({
        project: projectFactory({ id: idFactory(), now: clock() }), dirty: true, status: 'ready', error: null,
        pendingIntent: null, selection: [], activeMaterialId: 'maple',
      });
      return { ok: true };
    }
    if (intent.type === 'open') {
      try {
        ...
        update({ project, dirty: false, status: 'ready', error: null, pendingIntent: null, selection: [], activeMaterialId: project.palette[0].id });
        return { ok: true, value: project };
      } catch (error) {
        const normalized = publicError(error);
  ...
  function request(intent) {
    if (state.dirty) { update({ pendingIntent: intent, error: null }); return { ok: true, pending: true }; }
    return performIntent(intent);
  }
  function newProject() { return request({ type: 'new' }); }
  function requestOpen(id) { return request({ type: 'open', id }); }

  function resolvePending(decision) {
    const intent = state.pendingIntent;
    if (!intent) return { ok: true };
    if (decision === 'cancel') { update({ pendingIntent: null }); return { ok: true }; }
    if (decision === 'save') {
      const saved = save();
  ...
  function setSelection(selection) { update({ selection: [...selection] }); }
  function setTool(tool) { update({ tool }); }
  function setActiveMaterial(materialId) { update({ activeMaterialId: materialId }); }
  ...
  return { getState, subscribe, refreshProjects, dispatch, save, newProject, requestOpen, resolvePending, exportImage, setSelection, setTool, setActiveMaterial, renameProject };
}
